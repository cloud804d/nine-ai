...
const robotAvatar = computed(() => {
  return activeGroupInfo.value?.appLogo || authStore.globalConfig.robotAvatar
})
...
<template>
  <template v-if="image">
    <NAvatar v-if="isString(avatar) && avatar.length > 0" :size="isMobile ? 32 : 42" :src="avatar" :fallback-src="defaultAvatar" />
    <NAvatar v-else :size="isMobile ? 32 : 42" :src="defaultAvatar" />
  </template>
  <span v-else class="text-[28px] dark:text-white">
    <NAvatar v-if="isString(robotAvatar) && robotAvatar.length > 0" style="background-color: transparent;" :size="isMobile ? 32 : 32" :src="robotAvatar" :fallback-src="defaultAvatar" />
