...
const submitDisabled = computed(() => {
  return !prompt.value || associateLoading.value || translateLoading.value || translateNoLoading.value
})
...
async function queryAllDrawList() {
  const res: ResData = await fetchMidjourneyDrawList({ page: 1, size: size.value })
...
async function handleAssociatePrompt() {
  ...
  try {
    const res: ResData = await fetchGetMjPromptAssociateApi({ prompt: prompt.value })
    if (!res.success)
      return ms.error('联想失败了')
    prompt.value = res.data
...
async function handleSubmit() {
  ...
  /* 如果使用我们的参数 则去掉用户自己的 */
  if (carryOptions.value)
    prompt.value = removeParameters(prompt.value)
...
async function queryDrawResult() {
  ...
  totalCount.value = count
  countQueue.value = queueCount || 0
  const curDrawTask = drawList.value.filter((item: any) => [1, 2].includes(item.status))
  if (curDrawTask.length)
    timer = setTimeout(() => queryDrawResult(), 3000)
...
<template>
  <div class="grow flex h-screen flex-col lg:pt-0">
    <div class="flex grow flex-col sm:flex-row h-full">
      <div class="p-4 sm:pt-6 bg-[#f8f8f8]  p-4 dark:bg-[#18181c] overflow-y-auto w-full sm:w-[20rem]  shrink-0 border-r-2 border-[#ffffff17]">
        ...
        <div class="flex mt-2 py-1 pb-2 space-x-1 overflow-x-auto justify-between scrollbar-none ">
          <button
          ...
          >
            <div class=" border-2   border-gray-300 box-borde rounded-md dark:bg-black flex flex-col items-center" :class="[activeAspect(item.aspect) ? 'aspect-active' : '', isMobile ? 'py-3' : 'py-2']">
              <div class="flex items-center justify-center w-6 h-6">
                <div class="border-gray-300 rounded border-2 " :class="[activeAspect(item.aspect) ? 'aspect-active' : '']" :style="{ width: item.width, height: item.height }" />
              </div>
        ...
        <ul class="mt-2 flex justify-between">
          ...
          >
            <button class="relative w-full h-full dark:bg-black  rounded" type="button">
              <div class="absolute w-full h-full flex justify-center items-center">
                <div class="text-2xl text-white font-bold absolute left-5 top-1">
                  {{ item.name }}
                </div>
              </div><img :src="item.img" class="rounded aspect-[3/1] w-full object-cover">
            </button>
          </li>
        </ul>
        <div class="mt-4">
          ...
          <div v-if="model === 'NIJI'" class="mt-2 flex justify-between items-center space-x-2 text-xs">
          ...
          <div v-if="model === 'MJ'" class="mt-3 flex justify-between items-center space-x-2 text-xs">
          ...
          <div class="mt-3 flex justify-between items-center space-x-2 text-xs">
            ...
            <span class="flex-1">
              <NSwitch v-model:value="carryOptions" size="small" :checked-value="1" :unchecked-value="0" />
            </span>
            <NTooltip placement="right-end" trigger="hover">
              <template #trigger>
                <SvgIcon icon="ri:error-warning-line" class="text-base ml-2" />
            ...
            <div class="ant-spin-container">
              <div class="mt-2 flex justify-center items-center dark:bg-black p-5 rounded-md">
                <label v-if="!dataBase64" for="upload-file">
                  <div class="upload cursor-pointer" @dragover.prevent @dragenter.prevent @dragleave.prevent @drop="handleDrop">
                    <input id="upload-file" type="file" accept=".png, .jpg, .jpeg" style="display: none;" @change="handleFileSelect($event)">
      ...
      <!-- 右 -->
      <div class="h-full flex-1" :class="[isMobile ? '' : 'overflow-y-auto overflow-hidden']">
            ...
            <div>
              <div class="flex justify-between items-end">
                ...
                <NSpace>
                  ...
                  <NButton type="primary" :loading="associateLoading" @click="handleAssociatePrompt">
                    <template #icon>
                      <SvgIcon icon="material-symbols:mindfulness-outline-rounded" class="text-base" />
                    </template>
                    联想
                  </NButton>
                </NSpace>
