...
<template>
  <div class="relative flex justify-center ">
    <div class="w-full flex flex-col items-center  max-w-screen-4xl p-4 lg:p-6  ">
      <div class="mb-2 mt-8 text-center text-3xl font-extrabold text-[#5A91FC] dark:text-[var(--primary-color-dark)] lg:text-4xl">
        {{ siteRobotName }}
      </div>
      ...
      <div class="w-full flex justify-center my-3 " :class="isMobile ? 'space-x-1' : 'space-x-5'">
        <NButton v-if="isShowBtn('/chat')" trong secondary type="primary" round @click="handlJumpPath('/chat')">
          <SvgIcon icon="carbon:chat" class="text-2xl mr-2" />
          AI 对话
        </NButton>
        <NButton v-if="isShowBtn('/midjourney')" trong secondary type="primary" round @click="handlJumpPath('/midjourney')">
          <SvgIcon icon="ph:pencil-slash-duotone" class=" text-2xl mr-2 " />
          AI 绘画
        </NButton>
        <NButton v-if="isShowBtn('/mind')" trong secondary type="primary" round @click="handlJumpPath('/mind')">
      ...
      <div class="mt-3 flex flex-col  justify-center w-full items-center overflow-hidden">
        <NInput v-model:value="keyword" class="!max-w-screen-4xl" round placeholder="搜索应用名称、快速查找应用...">
          <template #suffix>
      ...
      <div class="sticky left-0 mt-3 right-0 top-0 flex max-w-screen-4xl  items-center z-50  w-full bg-[#f3f4fc] dark:bg-[#101014]">
        <NScrollbar x-scrollable class="!h-[60px] ">
          <div class="flex mt-3 items-center space-x-3 whitespace-nowrap">
            <NButton v-for="(item, index) in catList" :key="index" size="small" round :type="activeCatId === item.id ? 'primary' : 'default'" @click="handleChangeCatId(item.id)">
              {{ item.name }}
            </NButton>
          </div>
        </NScrollbar>
