...
<template>
  <div class="flex min-w-[75px]  bg-[#e8eaf1] pb-2 dark:bg-[#25272d]  dark:border-[#3a3a40]    border-#efeff5-800 " :class="getMobileLayoutClass" :style="mobileSafeArea">
    ...
    <main ref="track" class="flex-1 flex-grow-1 mb-5 overflow-auto" :class="[getIconMobileLayoutClass]">
      <NScrollbar :size="1">
        <div class="flex h-full flex-col items-center space-y-3">
          <div v-for="item in menuList" :key="item.menuName" class="flex justify-center flex-col items-center" :class="isMobile ? 'mt-0' : 'mt-3'" @click="handleClickMenu(item)">
            <div class="h-12 w-12 border  cursor-pointer bg-white dark:bg-[#34373c] rounded-lg duration-300 flex justify-center items-center" :class="[isActive(item) ? 'border border-[#73a2ff] dark:border-[#73a2ff]' : 'border-transparent']">
              <SvgIcon :icon="item.menuIcon" class="text-2xl  transition-all" :class="[isActive(item) ? 'text-[#3076fd] dark:text-[#3076fd] ' : '']" />
            </div>
            <span class="text-[12px] mt-1 margin-auto  whitespace-nowrap overflow-hidden" :class="[isActive(item) ? 'text-[#3076fd] dark:text-[#3076fd] ' : 'text-[#999999]']">{{ item.menuTipText }}</span>
          </div>
        </div>
      </NScrollbar>
    </main>
