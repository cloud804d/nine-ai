...
import { NButton, NCascader, NCollapse, NCollapseItem, NIcon, NInput, NModal, NSlider, NTooltip, useMessage } from 'naive-ui'
...
const modelMapsCache: any = ref({})
const modelTypeListCache: any = ref([])
...
const options = computed(() => {
  const data = !activeGroupAppId.value ? modelTypeListCache : modelTypeListCache.value.filter((item: any) => Number(item.val) === 1)
...
<template>
  <NModal
    :show="visible" style="width: 90%; max-width: 650px" :on-after-enter="openDialog"
  ...
  >
    <div class="py-3 px-5 bg-white rounded dark:bg-slate-800">
      ...
      <div class=" pb-4">
        ...
        <NInput v-model:value="systemMessage" type="textarea" :disabled="disabled" placeholder="自定义头部预设、给你的AI预设一个身份、更多有趣的角色请前往「应用广场」..." />
      </div>
      ...
      <div class="flex justify-between items-center mt-6">
        ...
        <div>
          <NCascader
            v-model:value="model" class="w-full" placeholder="请选用当前聊天组所需的模型！" expand-trigger="click"
            :options="options" check-strategy="child" :show-path="true" :filterable="true"
          />
        </div>
      </div>
            ...
            <div class="mt-2">
              <div>
                <div class=" w-full flex justify-between">
                  ...
                  <div class="flex w-[200px] items-center">
                    <NSlider v-model:value="topN" :step="0.1" :max="maxTemperature" />
              ...
              <div class="mt-4">
                <div class=" w-full flex justify-between">
                  ...
                  <div class="flex w-[200px] items-center">
                    <NSlider v-model:value="maxResponseTokens" :step="100" :max="maxModelTokens" />
                    <span class="w-[55px] text-right">
                      {{ maxResponseTokens }}
                    </span>
                  </div>
              ...
              <div class="mt-4">
                <div class=" w-full flex justify-between">
                  ...
                  <div class="flex w-[200px] items-center">
                    <NSlider v-model:value="rounds" :step="1" :max="maxRounds" />
                    <span class="w-[55px] text-right">
                      {{ rounds }}
                    </span>
                  </div>
