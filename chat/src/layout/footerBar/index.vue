...
<template>
  <footer class="bg-white dark:bg-[#25272c]">
    <div class="grid border-t py-1 dark:border-t-neutral-800 grid-cols-2" :style="{	gridTemplateColumns: `repeat(${menuLista.length}, minmax(0, 1fr))` }">
      <a v-for="(item) in menuLista" :key="item.id" :class="[isActive(item) ? 'text-[#3076fd] dark:text-[#3076fd]' : '']" class="cursor-pointer text-center leading-4 " @click="handleToPage(item)">
