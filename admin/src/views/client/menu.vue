  ...
  <div>
    ...
    <el-card style="margin: 20px;">
      ...
      <el-table :data="menuList" style="width: 100%;">
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <div class="flex items-center">
              <el-button v-if="scope.row.menuType" size="small" @click="handleEditMenu(scope.row)">
                编辑菜单
              </el-button>
              <el-button v-if="!scope.row.menuType" size="small" @click="handleEditIcon(scope.row)">
                修改默认
              </el-button>
              <el-button v-if="!scope.row.menuType" :type="scope.row.isShow ? 'danger' : 'success'" size="small" @click="handleShowMenu(scope.row)">
                {{ scope.row.isShow ? '隐藏菜单' : '打开菜单' }}
              </el-button>
              <el-popconfirm v-if="scope.row.menuType" title="是否删除当前菜单？" @confirm="handleDelete(scope.row)">
                <template #reference>
    ...
    <el-dialog v-model="visible" :close-on-click-modal="false" :title="dialogTitle" width="570" @close="handlerCloseDialog(formRef)">
      <el-form ref="formRef" label-width="140" :rules="rules" :model="formInline">
        ...
        <el-form-item v-if="formInline.isSystem" label="加载三方网页地址" prop="menuIframeUrl">
        ...
        <el-form-item v-if="!formInline.isSystem" label="系统内菜单路径" prop="menuPath">
